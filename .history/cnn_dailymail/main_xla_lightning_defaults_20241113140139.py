...
class T5SummarizationModule(pl.LightningModule):
    def __init__(self, model_name, learning_rate, optimizer_name="adamw", generation_max_tokens=20, bert_score_model_to_use="microsoft/deberta-large-mnli"):        
        super().__init__()
...
def main(seed, optimizer_name, batch_size, training_mode="None"):
    ic.disable()
    print(f"Training with seed {seed}, optimizer {optimizer_name}, batch size {batch_size}, and default learning rate")
    ...
    wandb_logger = WandbLogger(project="t5_summarization_project",
                               name=f"{model_name}_{optimizer_name}_seed_{seed}_default_mode")

    checkpoint_callback = ModelCheckpoint(dirpath= f"checkpoints/{model_name}_{optimizer_name}_seed_{seed}_default_mode", 
                                          monitor="val_loss", 
                                          mode="min",
                                          save_last=True)
    ...
    hyperparameters = dict(learning_rate="default", 
                           optimizer_name=optimizer_name, 
                           seed_num=seed, 
                           dataset_name=dataset_name, 
                           model_name=model_name, 
...
if __name__ == "__main__":
    ...
    main(args.seed, args.optim, args.batch_size, args.training_mode) # **optimizer_params