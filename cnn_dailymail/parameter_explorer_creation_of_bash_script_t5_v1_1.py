...
def read_best_hyperparameters(base_path='./hypertuning_results_full_training/google_t5-v1_1-small/'):
    hyperparams = {}
    ...
    if not os.path.exists(base_path):   # Depends on the directory structure
        base_path = "./hypertuning_results_lr_tuning/google_t5-v1_1-small/"
...
def generate_bash_script(hyperparams, training_mode):
    ...
    for optimizer_name, seeds_data in hyperparams.items():
        ...
        for seed_dir, params in seeds_data.items():
            ...
            command = f"PJRT_DEVICE=TPU python main_xla_lightning_t5_v1_1.py --seed {seed} --optim {optimizer_name} --batch_size 8 --learning_rate {learning_rate}"
